...
A cops and robbers game will take in a graph G and a natural number n (the number of cops)
-/
structure cr_game {V W: Type}[fintype V] (G: refl_graph V) (n: fin (fintype.card V)) :=
{cop_strat: option (vector V n) → option V → option (vector V n)}
{cop_: }
{robber_strat: option (vector V n) → option V → option V}
section CR_graphs
